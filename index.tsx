...
/**
 * Halal Digital Services - Version 5.4
 * Feature: Unified Social Design (Footer matches Article Share style)
 */
...
const loadState = () => {
    try {
        ...
        // MIGRATION: Force update links if they belong to "halaldigital" or are from an older version
        const storedVersion = localStorage.getItem('app_version');
        if (storedVersion !== APP_VERSION || JSON.stringify(settings.socialLinks).includes('halaldigital')) {
            settings.socialLinks = { ...INITIAL_SETTINGS.socialLinks };
            settings.whatsappNumber = INITIAL_SETTINGS.whatsappNumber;
        ...
        footer.innerHTML = `
            <footer class="bg-gray-900 dark:bg-black text-white py-12 md:py-24 mt-20 border-t border-gray-800">
                <div class="max-w-7xl mx-auto px-6 grid grid-cols-1 md:grid-cols-3 gap-12 text-center md:text-right">
                    <div class="space-y-6">
                        <div class="text-3xl font-black">${state.settings.siteName.split(' ')[0]} <span class="text-blue-500">${state.settings.siteName.split(' ').slice(1).join(' ') || 'ديجيتال'}</span></div>
                        <p class="text-gray-400 text-sm md:text-base leading-relaxed">وكالة رقمية متخصصة في مساعدة المقاولات والشباب في المغرب على دخول عالم التجارة الإلكترونية بنجاح.</p>
                    </div>
                    <div class="space-y-6">
                        <h4 class="text-xl font-black text-blue-500">روابط سريعة</h4>
                        <ul class="flex flex-col gap-3 text-gray-400 font-bold text-sm">
                    ...
                    <div class="space-y-8">
                        <h4 class="text-xl font-black text-blue-500">تواصل معي</h4>
                        <div class="flex justify-center md:justify-start gap-4 text-white">
                            <a href="${state.settings.socialLinks.facebook}" target="_blank" class="w-12 h-12 flex items-center justify-center bg-blue-600 rounded-full hover:scale-110 transition shadow-lg" title="Facebook">${SOCIAL_ICONS.facebook}</a>
                            <a href="${state.settings.socialLinks.twitter}" target="_blank" class="w-12 h-12 flex items-center justify-center bg-black rounded-full hover:scale-110 transition shadow-lg" title="X (Twitter)">${SOCIAL_ICONS.twitter}</a>
                            <a href="${state.settings.socialLinks.instagram}" target="_blank" class="w-12 h-12 flex items-center justify-center bg-gradient-to-tr from-yellow-500 to-purple-600 rounded-full hover:scale-110 transition shadow-lg" title="Instagram">${SOCIAL_ICONS.instagram}</a>
                            <a href="${state.settings.socialLinks.telegram}" target="_blank" class="w-12 h-12 flex items-center justify-center bg-blue-400 rounded-full hover:scale-110 transition shadow-lg" title="Telegram">${SOCIAL_ICONS.telegram}</a>
                            <a href="${state.settings.socialLinks.pinterest}" target="_blank" class="w-12 h-12 flex items-center justify-center bg-red-600 rounded-full hover:scale-110 transition shadow-lg" title="Pinterest">${SOCIAL_ICONS.pinterest}</a>
                        </div>
                        <div class="text-gray-500 font-bold text-xs mt-4">واتساب: ${state.settings.whatsappNumber}</div>
                    </div>
                </div>
                <div class="text-center pt-20 text-gray-700 text-[10px] font-black uppercase tracking-widest">
...
const renderAdUnit = (type: 'adsHeader' | 'adsMiddle' | 'adsBottom', label: string) => {
    ...
    if (adCode && adCode.trim() !== '') return `<div class="my-10 flex justify-center ad-container">${adCode}</div>`;
    return `<div class="my-10 p-6 bg-gray-50 dark:bg-gray-900 border-2 border-dashed border-gray-200 dark:border-gray-800 rounded-2xl text-center text-gray-300 text-xs font-bold uppercase">إعلان ${label}</div>`;
};
        ...
        <div class="space-y-16 md:space-y-32 animate-fadeIn pb-10 text-right">
            <section class="relative min-h-[500px] flex items-center bg-gray-950 text-white overflow-hidden px-4">
                ...
                <div class="max-w-7xl mx-auto relative z-10 w-full py-16 text-right">
                    <div class="max-w-4xl space-y-10">
                        ...
                        <button onclick="window.location.hash='#/blog'" class="bg-blue-600 hover:bg-blue-700 text-white px-12 py-5 rounded-2xl font-black text-xl transition-all shadow-xl">تصفح المدونة التقنية</button>
                    </div>
                </div>
            </section>
            ...
            <section class="max-w-7xl mx-auto px-6">
                <div class="flex justify-between items-end mb-16">
                    <h2 class="text-4xl font-black dark:text-white">جديد المدونة</h2>
                    <a href="#/blog" class="text-blue-600 font-black">كل المقالات ←</a>
                </div>
                <div class="grid grid-cols-1 md:grid-cols-3 gap-12">
                    ${state.articles.slice(0, 3).map((a: any) => `
                        <article onclick="window.location.hash='#/article/${a.id}'" class="bg-white dark:bg-gray-900 rounded-[3rem] overflow-hidden border border-gray-100 dark:border-gray-800 shadow-sm hover:shadow-lg transition cursor-pointer">
                            ...
                            <div class="p-8 space-y-4 text-right">
                                ...
                                <p class="text-gray-500 dark:text-gray-400 line-clamp-2">${a.excerpt}</p>
                            </div>
                        </article>
                    `).join('')}
                </div>
        ...
        <div class="max-w-4xl mx-auto px-6 py-20 animate-fadeIn text-right">
            ...
            <div class="prose prose-xl dark:prose-invert max-w-none text-gray-800 dark:text-gray-300 font-medium leading-relaxed">
                ${article.content.split('\n').map((p: string, i: number) => {
                    if (p.trim() === '') return '';
                    return `<p class="mb-6">${p}</p>${i === 2 ? renderAdUnit('adsMiddle', 'وسط المحتوى') : ''}`;
                }).join('')}
            </div>
...
(window as any).switchTab = (tab: string) => {
    ...
    if (tab === 'settings') {
        container.innerHTML = `
            <h2 class="text-3xl font-black mb-8 dark:text-white">إعدادات الموقع والتواصل</h2>
            <div class="bg-white dark:bg-gray-900 p-10 rounded-[2.5rem] space-y-10 text-right shadow-xl">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div><label class="block font-black mb-2 dark:text-gray-400">اسم الموقع</label><input id="set-name" value="${state.settings.siteName}" class="w-full p-4 bg-gray-50 dark:bg-gray-800 dark:text-white rounded-xl text-right border dark:border-gray-700"></div>
                    <div><label class="block font-black mb-2 dark:text-gray-400">رقم الواتساب</label><input id="set-wa" value="${state.settings.whatsappNumber}" class="w-full p-4 bg-gray-50 dark:bg-gray-800 dark:text-white rounded-xl text-right border dark:border-gray-700"></div>
                </div>
                <div class="space-y-6">
                    <h3 class="text-xl font-black text-blue-500 border-b pb-2">روابط التواصل الاجتماعي</h3>
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div><label class="block font-black mb-2 dark:text-gray-400">فيسبوك</label><input id="set-fb" value="${state.settings.socialLinks.facebook}" class="w-full p-4 bg-gray-50 dark:bg-gray-800 dark:text-white rounded-xl text-left font-mono text-sm" dir="ltr"></div>
                        <div><label class="block font-black mb-2 dark:text-gray-400">تويتر (X)</label><input id="set-tw" value="${state.settings.socialLinks.twitter}" class="w-full p-4 bg-gray-50 dark:bg-gray-800 dark:text-white rounded-xl text-left font-mono text-sm" dir="ltr"></div>
                        <div><label class="block font-black mb-2 dark:text-gray-400">إنستقرام</label><input id="set-ig" value="${state.settings.socialLinks.instagram}" class="w-full p-4 bg-gray-50 dark:bg-gray-800 dark:text-white rounded-xl text-left font-mono text-sm" dir="ltr"></div>
                        <div><label class="block font-black mb-2 dark:text-gray-400">تلجرام</label><input id="set-tg" value="${state.settings.socialLinks.telegram}" class="w-full p-4 bg-gray-50 dark:bg-gray-800 dark:text-white rounded-xl text-left font-mono text-sm" dir="ltr"></div>
                        <div><label class="block font-black mb-2 dark:text-gray-400">بنترست</label><input id="set-pn" value="${state.settings.socialLinks.pinterest}" class="w-full p-4 bg-gray-50 dark:bg-gray-800 dark:text-white rounded-xl text-left font-mono text-sm" dir="ltr"></div>
                    </div>
                </div>
                <button onclick="updateSettings()" class="w-full py-6 bg-blue-600 text-white rounded-2xl font-black shadow-xl hover:bg-blue-700 transition">حفظ التغييرات وتحديث الموقع ✅</button>
            </div>
        `;
    }
};
...
const router = () => {
    ...
    if (hash === '#/') root.innerHTML = renderHome();
        ...
        root.innerHTML = `<div class="max-w-7xl mx-auto px-6 py-20 text-right"><h1 class="text-5xl font-black dark:text-white mb-16">المدونة التقنية</h1><div class="grid grid-cols-1 md:grid-cols-3 gap-12">${state.articles.map((a: any) => `<article onclick="window.location.hash='#/article/${a.id}'" class="bg-white dark:bg-gray-900 rounded-[2.5rem] border overflow-hidden shadow-sm cursor-pointer"><img src="${a.image}" class="h-60 w-full object-cover"><div class="p-8"><h3 class="text-2xl font-black dark:text-white">${a.title}</h3><p class="text-gray-500 line-clamp-2">${a.excerpt}</p></div></article>`).join('')}</div></div>`;
        syncHeaderAndFooter();
    }
    else if (hash.startsWith('#/article/')) root.innerHTML = renderArticleDetail(hash.replace('#/article/', ''));
    else if (isDashboard) {
        if (sessionStorage.getItem('isAdmin') !== 'true') {
            root.innerHTML = `<div class="min-h-screen flex items-center justify-center bg-gray-50 dark:bg-gray-950"><div class="bg-white dark:bg-gray-900 p-10 rounded-[2.5rem] shadow-2xl space-y-8 w-full max-w-md text-center"><h2 class="text-2xl font-black dark:text-white">لوحة التحكم</h2><input type="password" id="dash-pass" class="w-full p-5 bg-gray-50 dark:bg-gray-800 dark:text-white rounded-2xl text-center font-bold" placeholder="كلمة السر"><button onclick="login()" class="w-full py-5 bg-blue-600 text-white rounded-2xl font-black shadow-xl hover:bg-blue-700 transition">دخول</button></div></div>`;
        } else {
            root.innerHTML = `<div class="min-h-screen bg-gray-50 dark:bg-gray-950 flex flex-col md:flex-row text-right"><aside class="w-full md:w-80 bg-gray-900 text-white p-10 flex flex-col"><div class="text-2xl font-black mb-12">الإدارة</div><nav class="flex flex-col gap-4"><button onclick="switchTab('settings')" class="text-right p-4 rounded-xl hover:bg-white/5 font-black">⚙️ الإعدادات</button><button onclick="window.location.hash='#/'" class="text-right p-4 rounded-xl hover:bg-white/5 font-black">🏠 رجوع للموقع</button></nav></aside><main class="flex-1 p-16 overflow-y-auto"><div id="dash-content" class="max-w-5xl mx-auto space-y-12"></div></main></div>`;
